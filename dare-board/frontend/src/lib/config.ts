...
export const IS_DEMO_MODE = !CONTRACT_ADDRESS;
...
export function normalizeAddress(address: string | bigint | null | undefined): string {
  if (address === null || address === undefined || address === "") {
    return ZERO_ADDRESS;
  }

  try {
    const normalized = BigInt(address.toString()).toString(16).padStart(64, "0");
...
export function shortAddress(address: string | bigint | null | undefined): string {
  const normalized = normalizeAddress(address);
  return `${normalized.slice(0, 6)}...${normalized.slice(-4)}`;
}

export function formatAmount(value: bigint | string | number | null | undefined): string {
