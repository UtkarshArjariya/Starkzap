...
import { ModernProfilePage } from "@/components/new-look/profile-page";
...
export default function ProfilePage() {
  const { mode } = useUI();
  if (mode === "modern") return <ModernProfilePage />;
        ...
        <main className="mx-auto max-w-4xl px-4 py-24 text-center sm:px-6">
          <div className="surface-panel px-6 py-12">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-400">
              Your profile shows the dares you posted, the ones you claimed, and
              the reputation trail created by your on-chain activity.
            </p>
            <button
  ...
  const activity = useMemo(() => {
    ...
    for (const d of dares) {
      ...
      if (isPoster) {
        events.push({
          type: "created",
          description: `Posted "${d.title}" \u2014 ${formatAmount(d.rewardAmount, getTokenDecimals(d.rewardToken))} ${getTokenSymbol(d.rewardToken)} at stake`,
          dare: d,
          timestamp: d.deadline - 86400,
        });
      }
      if (isClaimer) {
        events.push({
          type: "claimed",
          description: `Claimed "${d.title}"`,
          dare: d,
          timestamp: d.deadline - 43200,
        });
      }
      if (isClaimer && d.proofSubmittedAt > 0) {
        events.push({
          type: "proof",
          description: `Submitted proof for "${d.title}"`,
          dare: d,
          timestamp: d.proofSubmittedAt,
        });
      }
      if (isClaimer && d.status === "Approved") {
        events.push({
          type: "won",
          description: `Won ${formatAmount(d.rewardAmount, getTokenDecimals(d.rewardToken))} ${getTokenSymbol(d.rewardToken)} on "${d.title}"`,
          dare: d,
          timestamp: d.votingEnd,
        });
      }
      if (isPoster && (d.status as string) === "Cancelled") {
        events.push({
          type: "cancelled",
          description: `Cancelled "${d.title}" \u2014 reward returned`,
          dare: d,
        ...
        <section className="surface-panel px-6 py-6">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div>
                ...
                {walletType === "privy" ? (
                  <p className="mt-2 text-xs text-amber-200/80">
                    This is a Privy-managed wallet. To post dares, send STRK
                    tokens to this address first. The paymaster only covers gas
                    fees.
                  </p>
                ) : null}
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              />
              <StatCard
                icon={<Trophy className="h-4 w-4 text-amber-100" />}
                label="Dares claimed"
        ...
        <section className="mt-6">
          {loading ? (
            ...
            activity.length === 0 ? (
              <div className="surface-panel px-6 py-12 text-center">
                ...
                <p className="mt-2 text-sm text-slate-400">
                  Post or claim a dare to start building your on-chain history.
                </p>
              </div>
            ) : (
            ...
            <div className="surface-panel px-6 py-12 text-center">
              <p className="text-lg font-semibold text-white">
                {tab === "posted"
                  ? "You have not posted any dares yet"
                  : "You have not claimed any dares yet"}
              </p>
              <p className="mt-2 text-sm text-slate-400">
                {tab === "posted"
